 ...
 * @see RegistryEvent
 */
public class ForgeSpecificMatrix implements PlatformSpecificMatrix {
    /**
     *
    ...
    public < T extends IForgeRegistryEntry<T>> void register(Class<?> clazz, RegistryEvent.Register<T> evt) {
     ...
     */
    @SuppressWarnings({"unchecked"})
    public < T extends IForgeRegistryEntry<T>> void register(Class<?> clazz, IEventBus bus) {
        Registrar registrar = clazz.getAnnotation(Registrar.class);
        if (registrar == null) {
            return;
        }
...
//                        field.set(value, registerer.register(field.getAnnotation(RegistryEntry.class).value(), () -> value));
    ...
    public void register(Class<?> clazz, Registry<?> registry) {
        Registrar registrar = clazz.getAnnotation(Registrar.class);
        if (registrar == null) {
            return;
        }
